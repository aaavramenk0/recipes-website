// GET API request sample: http://localhost:3000/api/recipes/id
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const handleCase: ResponseFunctions = {
        ...
        GET: async (req: NextApiRequest, res: NextApiResponse) => {
            ...
            res.json(await Recipe.findById(id).catch(catcher));
        },
        // response for post requests
        PUT: async (req: NextApiRequest, res: NextApiResponse) => {
            const { Recipe } = await connect() // connect to database
            res.json(
                await Recipe.findByIdAndUpdate(id, req.body, { new: true }).catch(catcher)
            );
        },
        // response for delete requests
        DELETE: async (req: NextApiRequest, res: NextApiResponse) => {
            const { Recipe } = await connect() // connect to database
            res.json(await Recipe.findByIdAndDelete(id).catch(catcher));
        },
    }
